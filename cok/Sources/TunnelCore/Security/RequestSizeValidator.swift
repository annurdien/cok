...
public struct RequestSizeValidator: Sendable {
    ...
    public static func validateHeaderValue(_ value: String) throws {
        guard value.count <= maxHeaderValueLength else { throw ValidationError.headerValueTooLong(length: value.count, maximum: maxHeaderValueLength) }
    }
    ...
    public static func validateSubdomain(_ subdomain: String) throws {
        guard subdomain.count <= maxSubdomainLength else { throw ValidationError.subdomainTooLong(length: subdomain.count, maximum: maxSubdomainLength) }
    }
