...
public actor AuthService {
    ...
    public func validate(apiKey: String, for subdomain: String) throws {
        guard let key = validateAPIKey(apiKey) else {
            throw ClientError.authenticationFailed
        }

        guard key.subdomain == subdomain else {
            throw ClientError.invalidSubdomain(subdomain)
        }
    }
